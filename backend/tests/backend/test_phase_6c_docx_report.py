...
def extract_docx_text(docx_bytes: bytes) -> str:
    """Extracts all paragraph and table text from a docx byte stream."""
...
def test_1_and_2_valid_docx_generation_and_zip_structure():
    # 1 & 2. Valid DOCX generation and valid Office Open XML ZIP structure
    ...
    # Valid ZIP archive containing word/document.xml
...
def test_3_and_4_report_id_and_exact_disposition_rendering():
    ...
    session = InspectionSession(
        inspection_id="sess_fail_docx_test",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
        capture_plan_id="plan_software_1",
        capture_status="COMPLETE_EVIDENCE_CAPTURE",
    ...
    assert "sess_fail_docx_test" in text
...
def test_5_incomplete_inspection_disposition_rendered():
    ...
    session = InspectionSession(
        inspection_id="sess_incomp_docx",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
        capture_plan_id="plan_software_1",
...
def test_8_and_9_visual_findings_and_not_applicable():
    ...
    session = InspectionSession(
        inspection_id="sess_na_vis_docx",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
        capture_plan_id="plan_software_1",
        capture_status="COMPLETE_EVIDENCE_CAPTURE",
...
def test_10_and_11_unknown_context_and_legal_references():
    ...
    session = InspectionSession(
        inspection_id="sess_unknown_docx",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
...
def test_12_snapshot_is_not_recomputed_determinism():
    ...
    snapshot = generate_inspection_report(session, TEST_PLAN)
    docx1 = generate_docx_report(snapshot)
    docx2 = generate_docx_report(snapshot)
    
    text1 = extract_docx_text(docx1)
    text2 = extract_docx_text(docx2)
    assert text1 == text2

def test_13_and_14_no_arbitrary_score_and_no_fully_compliant():
    # 13 & 14. Confirm no compliance score or "fully compliant" in DOCX
    session = InspectionSession(
        inspection_id="sess_safe_docx",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
        capture_plan_id="plan_software_1",
        capture_status="COMPLETE_EVIDENCE_CAPTURE",
...
def test_15_and_16_long_strings_and_multipage():
    # 15 & 16. Long strings and multi-item tables in DOCX
...
def test_17_pdf_and_docx_consistency():
    # 17. Verify that PDF and DOCX generated from the same snapshot contain consistent substantive data
    rule1 = RuleEvaluationResult(
        rule_id="MRP_DECLARATION_PRESENCE",
        rule_version="1.0",
        field="MRP",
    ...
    session = InspectionSession(
        inspection_id="sess_consistent_01",
        reference_date="2025-01-01",
        product_category="GENERIC_RETAIL_PACKAGE",
    ...
    # Substantive invariants present in both
